...
export const getParsedErrorWithAllAbis = (error: any, chainId: AllowedChainIds): string => {
  ...
  if (/Encoded error signature.*not found on ABI/i.test(originalParsedError)) {
    ...
    try {
      ...
      // `contractsData` (deployed + external) so error selectors registered in
      // `externalContracts.ts` (e.g. OZ v5 ERC20 custom errors like
      // ERC20InsufficientAllowance / ERC20InsufficientBalance from USDC and
      // CLAWD) can be resolved when they bubble up through CLAWDdca reverts.
